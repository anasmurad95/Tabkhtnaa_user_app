...
class _OrderMealScreenState extends State<OrderMealScreen> {
  ...
  Future<void> _loadSettings() async {
    ...
    try {
      ...
      final settings = await SettingsRepository(context.read<ApiClient>()).getSettings();
  ...
  double get _taxRate {
    ...
    if (_settings == null) return 0.15;

    return SettingsRepository(context.read<ApiClient>()).taxPercentage(_settings!) / 100;

  }



  double get _vat => _subtotal * _taxRate;



  double get _deliveryFee {

    if (_delivery != 'delivery' || _settings == null) return 0;

    return SettingsRepository(context.read<ApiClient>()).deliveryFee(_settings!);
  ...
  Future<void> _confirm() async {
    ...
    final ok = await cartProvider.add(
    ...
    final details = _notes.text.trim().isEmpty

        ? null

        : '${context.tr('meal_time', fallback: 'وقت الوجبة')}: ${_mealTime.format(context)}\n${_notes.text.trim()}';
